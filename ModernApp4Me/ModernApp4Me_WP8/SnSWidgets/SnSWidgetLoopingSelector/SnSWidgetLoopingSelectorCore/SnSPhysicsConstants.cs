...
namespace ModernApp4Me_WP8.SnSWidgets.SnSWidgetLoopingSelector.SnSWidgetLoopingSelectorCore
{
    internal static class SnSMotionParameters
    {
    ...
    internal static class SnSPhysicsConstants
    {               
        internal static double GetStopTime(Point initialVelocity)
        {
            ...
            //
            if (SnSMotionParameters.ParkingSpeed >= initialVelocityMagnitude)
            {
                return 0;
            }
            
            return Math.Log(SnSMotionParameters.ParkingSpeed / initialVelocityMagnitude) / Math.Log(SnSMotionParameters.Friction);
        }
        ...
        internal static Point GetStopPoint(Point initialVelocity)
        {
            ...
            if (initialVelocityMagnitude > SnSMotionParameters.MaximumSpeed && initialVelocityMagnitude > 0)
            {
                ...
                cappedInitialVelocity.X *= SnSMotionParameters.MaximumSpeed / initialVelocityMagnitude;
                cappedInitialVelocity.Y *= SnSMotionParameters.MaximumSpeed / initialVelocityMagnitude;
            }
            ...
            //
            double initialVelocityCoefficient = (Math.Pow(SnSMotionParameters.Friction, GetStopTime(cappedInitialVelocity)) - 1) / Math.Log(SnSMotionParameters.Friction);
        ...
        internal static IEasingFunction GetEasingFunction(double stopTime)
        {
            ...
            ee.Exponent = stopTime * Math.Log(SnSMotionParameters.Friction);
